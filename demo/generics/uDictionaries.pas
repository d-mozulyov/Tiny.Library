...
type
  ...
  public type
    ...
    SystemTiny = class(SystemSystem)
      constructor Create(const Items: TItems; const Capacity: Integer); override;
    end;

    TinyTiny = class(TTest)
      Dictionary: Tiny.Generics.TDictionary<T,Integer>;
...
{ TRunner<T>.SystemTiny }

constructor TRunner<T>.SystemTiny.Create(const Items: TItems; const Capacity: Integer);
var
  i: Integer;
  Comparer: Generics.Defaults.IEqualityComparer<T>;
begin
  IInterface(Comparer) := Tiny.Generics.TEqualityComparer<T>.Default;
...
constructor TRunner<T>.TinyTiny.Create(const Items: TItems; const Capacity: Integer);
var
  i: Integer;
begin
  Dictionary := Tiny.Generics.TDictionary<T,Integer>.Create(Capacity);
  for i := Low(TItems) to High(TItems) do
    Dictionary.AddOrSetValue(Items[i], i);
end;

destructor TRunner<T>.TinyTiny.Destroy;
begin
  Dictionary.Free;
  inherited;
end;

function TRunner<T>.TinyTiny.ExecuteItems(const Items: TItems): Integer;
var
  i: Integer;
begin
  for i := Low(TItems) to High(TItems) do
